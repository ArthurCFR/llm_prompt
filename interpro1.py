...
# --- NEW: Utility function for processing injected use case data ---
def _preprocess_injected_use_case_data(injected_config):
    """
    Prepares a single use case configuration from an injected JSON.
    Ensures essential fields are present and correctly typed/initialized.
    """
    ...
    if "template" not in processed_config or not isinstance(processed_config["template"], str):
        # If template is missing or invalid, this use case cannot be processed.
        # This check should ideally be done before calling this function.
        # For safety, return None or raise an error. Here, we'll make it an empty string.
    ...
    # Ensure 'variables' is a list, and process them
    ...
    for var_info in processed_config.get("variables", []):
        if isinstance(var_info, dict):
            # Validate variable structure (minimal: name, label, type)
            ...
            # Date default value handling (string to date object if needed for consistency, then back to string for saving)
            if var_info.get("type") == "date_input":
                if "default" in var_info and isinstance(var_info["default"], str):
                    try:
                        # Store as string for JSON, but validate format
                        datetime.strptime(var_info["default"], "%Y-%m-%d") 
                    except ValueError:
                     ...
                     var_info["default"] = var_info["default"].strftime("%Y-%m-%d") # Already a date object, format for saving
                else: # No default or invalid type
                    var_info["default"] = datetime.now().date().strftime("%Y-%m-%d")


            # Number input default, min, max, step handling (ensure float)
            if var_info.get("type") == "number_input":
                for num_field in ["default", "min_value", "max_value", "step"]:
                    if num_field in var_info and var_info[num_field] is not None:
                        try:
                            var_info[num_field] = float(var_info[num_field])
                        except (ValueError, TypeError):
                            ...
                            else: var_info[num_field] = None # For min/max
                    elif num_field == "default" and (num_field not in var_info or var_info[num_field] is None):
                        var_info[num_field] = 0.0 # Default to 0.0 if not present
                    elif num_field == "step" and (num_field not in var_info or var_info[num_field] is None):
                         var_info[num_field] = 1.0 # Default step to 1.0

            # Ensure options for selectbox is a list of strings
                ...
                if "default" not in var_info or var_info["default"] not in var_info["options"]:
                    if var_info["options"]:
                        ...
                        st.warning(f"Défaut de la variable selectbox '{var_info['name']}' non valide ou manquant. Premier option utilisée.")
                         ...
                         var_info["default"] = "" # No options, no default
    ...
    # Remove 'is_favorite' if present
    if "is_favorite" in processed_config: # pragma: no cover
        del processed_config["is_favorite"]
        
    # Other fields like 'previous_template' or 'height' for text_area variables
    # will be kept if present in injected_config.
...
# Default view mode logic - moved before specific view initializations
if 'view_mode' not in st.session_state:
    st.session_state.view_mode = "library" # Default to library
...
# --- NEW: Session state for Injection Feature ---
...
# --- MODIFIED: Added new tab for Injection ---
            ...
            if st.session_state.confirming_delete_family_name == current_selected_family_for_edit_logic:
                ...
                if st.button(button_text_confirm_delete, type="primary", key=f"confirm_del_fam_sb_{current_selected_family_for_edit_logic}", use_container_width=True):
                    ...
                    if st.session_state.library_selected_family_for_display == deleted_fam_name:
                        st.session_state.library_selected_family_for_display = None
                    st.session_state.view_mode = "edit" # Or library if preferred after delete
                    st.rerun()
...
with tab_injection:
    ...
    if st.button("🚀 Démarrer l'Injection", key="start_injection_btn", use_container_width=True):
        st.session_state.view_mode = "inject"
        st.session_state.injection_selected_family = None # Reset previous selection
        st.session_state.injection_json_text = "" # Reset previous text
        st.rerun()


# --- Main Display Area (Logic for Library, Edit, and NEW Injection views) ---
    ...
    if not final_selected_family_edition :
        ...
        with st.form(key=f"gen_form_{final_selected_family_edition}_{final_selected_use_case_edition}"):
            ...
            if st.form_submit_button("🚀 Générer Prompt"):
                ...
                try:
                    class SafeFormatter(dict): # pragma: no cover
           ...
           st.session_state.confirming_delete_details["use_case"] == final_selected_use_case_edition:
            ...
            if c1_del_uc.button(f"Oui, supprimer '{details['use_case']}'", key=f"del_yes_{details['family']}_{details['use_case']}", type="primary"):
                ...
                if st.session_state.editing_variable_info and \
                   ...
                   st.session_state.editing_variable_info.get("use_case") == deleted_uc_name_for_msg: # pragma: no cover
                    st.session_state.editing_variable_info = None 
                st.session_state.active_generated_prompt = ""
                st.session_state.variable_type_to_create = None
                st.session_state.view_mode = "edit"
            ...
            if st.session_state.editing_variable_info and \
               ...
               st.session_state.editing_variable_info.get("use_case") == final_selected_use_case_edition:
                ...
                if edit_var_idx < len(current_prompt_config.get('variables',[])):
                    ...
                    st.session_state.editing_variable_info = None 
                    st.session_state.variable_type_to_create = None 
                    st.warning("La variable que vous tentiez de modifier n'existe plus. Annulation de l'édition.") 
                    st.rerun() 
...
# --- NEW: Main Display Area for Injection View ---
elif st.session_state.view_mode == "inject":
    st.header("💉 Injection de Nouveaux Cas d'Usage")
    st.markdown("""
    Collez ici un ou plusieurs cas d'usage au format JSON. 
    Le JSON doit être un dictionnaire où chaque clé est le nom du nouveau cas d'usage, 
    et la valeur est sa configuration (template, variables, tags).
    Les champs `created_at`, `updated_at` et `usage_count` seront automatiquement initialisés.
    """)
    ...
    st.code("""
{
  "Nom de Mon Nouveau Cas d'Usage": {
    ...
    if not available_families_for_injection:
        st.warning("Aucune famille n'existe. Veuillez d'abord créer une famille via l'onglet 'Génération & Édition'.")
    else:
        # Select family for injection
        selected_family_for_injection = st.selectbox(
            "Choisissez la famille de destination pour l'injection :",
            options=[""] + available_families_for_injection,  # Add an empty option for placeholder
            index=0, # Default to empty
            key="injection_family_selector"
        )
        ...
        if st.session_state.injection_selected_family:
            ...
            st.session_state.injection_json_text = st.text_area(
                "Collez le JSON des cas d'usage ici :",
                value=st.session_state.get("injection_json_text", ""),
                height=300,
                key="injection_json_input"
            )
                    ...
                    try:
                        ...
                        if not isinstance(injected_data, dict):
                            ...
                            for uc_name, uc_config in injected_data.items():
                                ...
                                if not isinstance(uc_config, dict) or "template" not in uc_config:
                                ...
                                if uc_name_stripped in family_prompts:
                                    # Option: Overwrite, Skip, or Rename. For now, skip.
                                ...
                                # Preprocess and validate the individual use case config
                                processed_uc_config = _preprocess_injected_use_case_data(uc_config)
                                
                                if not processed_uc_config.get("template"): # Check if template became empty after processing
                                    failed_injections.append(f"'{uc_name_stripped}': Template invalide après traitement.")
                                    continue

                                family_prompts[uc_name_stripped] = processed_uc_config
                            ...
                            if successful_injections:
                                ...
                                st.session_state.injection_json_text = "" # Clear text area after successful injection
                                
                                # Optionally, switch to edit mode for the first injected prompt
                                if first_new_uc_name:
                                    ...
                                    st.session_state.go_to_config_section = True # Go to config section
                                    st.rerun()
                        ...
                        st.error(f"Une erreur inattendue est survenue lors de l'injection : {e}")
        else:
            st.info("Veuillez sélectionner une famille de destination pour commencer l'injection.")


else: # Fallback if view_mode is invalid or not set
    ...
    elif st.session_state.view_mode not in ["library", "edit", "inject"]: # pragma: no cover
        st.session_state.view_mode = "library" if list(st.session_state.editable_prompts.keys()) else "edit"
        st.rerun()

# --- Sidebar Footer ---
st.sidebar.markdown("---")
st.sidebar.info(f"Générateur v3.3.1 - © {CURRENT_YEAR} La Poste (démo)")
